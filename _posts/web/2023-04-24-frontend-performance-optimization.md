...
이처럼 중요한 것은 **주요 렌더링 결과를 최적화**하는 것이다. 즉, 렌더링 과정에서 브라우저가 다른 리소스에 의해 HTML 파싱이 지연되지 않도록 하여 페인팅을 빠르게 할 수 있도록 하는 것이 중요하다.

주요 렌더링 경로 최적화가 잘 된 페이지의 경우, 렌더 트리 구성이 빠른 시점에 이루어져 컨텐츠가 이른 시간에 점진적으로 보여지기 시작한다. 반면, 최적화가 되지 않은 페이지는 렌더링이 중단되어 긴 시간동안 빈 화면을 보여주다가 한 순간에 모든 컨텐츠가 보여진다. 이를 블록 리소스 최적화라고 하는데, 잠시 후에 알아보도록 하자.
...
<a href="https://user-images.githubusercontent.com/35218826/59728735-36851b80-9276-11e9-8a3d-ed60eaec2d1d.png">
  ...
  <figcaption>주요 렌더링 경로 최적화가 잘 된 웹페이지와 그러지 못한 웹페이지의 비교<br/>사용자는 최적화가 잘 된 페이지의 로딩이 더 빠르다고 느낀다.</figcaption>
</a>
</figure>
