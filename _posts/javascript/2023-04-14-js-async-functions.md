...
우선 `async` 라는 키워드부터 살펴보자. 이 키워드를 `function` 앞에 붙이면 비동기 함수를 선언할 수 있다. 이 **함수는 항상 프로미스 객체를 반환**하고, **리턴 값은 `resolve()`의 값**과 같다. reject하기 위해선 `throw Error()`
...
`await`을 쓰면서 코드는 보기 좋아졌지만, 단점이 생겼다. 비동기 함수들이 비효율적으로 하나씩 직렬로 처리되길 기다려야 한다는 것이다. 이를 병렬적으로 처리하는 방법이 두 가지 있다.
